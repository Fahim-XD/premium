...
class crack:
	...
	def main(self,fl):
		try:
			...
			self.ko+=1
			print("\033[1;96m[Crack] %s/%s | OK : %s | CP : %s"%(self.ko,len(self.fl),len(self.ada),len(self.cp)), end=' ');sys.stdout.flush()
		except:
			self.main(fl)
    ...
    if self.setpw == False:
      ...
      for pw in fl["pw"]:
        ...
        try:
          ...
          continue
        print(("\033[1;96m[Crack] %s/%s | OK : %s | CP : %s"%(self.loop,len(self.fl),len(self.ok),len(self.cp))), end=' ');sys.stdout.flush()
    else:
      self.loop += 1
...
def wangsaff():
    ...
    os.system("xdg-open https://wa.me/+8801880290192?text=Assalamualaikum%20Bang,%20Saya%20Pengguna%20SC%20SBF%20Dapunta.%20Code:%20404")
    input(k+"\n[ "+p+"Back"+k+" ]"+p)
    menu()
